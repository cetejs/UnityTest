...
public class Quaternion {
    ...
    public static Quaternion Euler(float x, float y, float z) {
        ...
        y *= Mathf.Deg2Rad;
        z *= Mathf.Deg2Rad;
        var qx = new Quaternion(Mathf.Sin(x / 2f), 0, 0, Mathf.Cos(x / 2f));
        var qy = new Quaternion(0, Mathf.Sin(y / 2f), 0, Mathf.Cos(y / 2f));
        var qz = new Quaternion(0, 0, Mathf.Sin(z / 2f), Mathf.Cos(z / 2f));
        return qy * qx * qz;
    }
    ...
    // 旋转轴取反, 同时角度也取反, 负负的正
    public static Quaternion Negative(Quaternion q) {
    ...
    public static Quaternion Exp(Quaternion q, float t) {
    ...
    public static Quaternion SlerpTheory(Quaternion a, Quaternion b, float t) {
        t = Mathf.Clamp01(t);
        var d = Diff(a, b);
    ...
    public static Matrix4x4 ToMatrix(Quaternion q) {
        ...
        var z = q.z;
        var m = new Matrix4x4();
        m.c0 = new Vector4(1 - 2 * y * y - 2 * z * z, 2 * x * y - 2 * w * z, 2 * x * z + 2 * w * y, 0);
        m.c1 = new Vector4(2 * x * y + 2 * w * z, 1 - 2 * x * x - 2 * z * z, 2 * y * z - 2 * w * x, 0);
        m.c2 = new Vector4(2 * x * z - 2 * w * y, 2 * y * z + 2 * w * x, 1 - 2 * x * x - 2 * y * y, 0);
