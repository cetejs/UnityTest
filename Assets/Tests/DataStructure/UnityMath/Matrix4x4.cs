...
public class Matrix4x4 {
    ...
    public Vector4 MultiplyPoint(Vector4 v) {
        var r = new Vector4();
        r.x = Vector4.Dot(c0, v);
        r.y = Vector4.Dot(c1, v);
        r.z = Vector4.Dot(c2, v);
        r.w = Vector4.Dot(c3, v);
        return r;
    }

    public Vector3 MultiplyPoint3X4(Vector3 v) {
        var r = MultiplyPoint(new Vector4(v.x, v.y, v.z, 1));
    ...
    private static Vector4 Multiply(Vector4 v, Matrix4x4 m) {
        var r = new Vector4();
    ...
    public static Matrix4x4 Transpose(Matrix4x4 m) {
        var r = new Matrix4x4();
        r.c0 = new Vector4(m.c0.x, m.c1.x, m.c2.x, m.c3.x);
        r.c1 = new Vector4(m.c0.y, m.c1.y, m.c2.y, m.c3.y);
        r.c2 = new Vector4(m.c0.z, m.c1.z, m.c2.z, m.c3.z);
        r.c3 = new Vector4(m.c0.w, m.c1.w, m.c2.w, m.c3.w);
        return r;
    }
    ...
    public static Matrix4x4 Rotate(Vector3 rot) {
        ...
        rm.c0 = new Vector4(cy * cz + sy * sx * sz, -cy * sz + sy * sx * cz, sy * cx, 0);
        rm.c1 = new Vector4(cx * sz, cx * cz, -sx, 0);
        rm.c2 = new Vector4(-sy * cz + cy * sx * sz, sy * sz + cy * sx * cz,  cy * cx, 0);
        rm.c3 = new Vector4(0, 0, 0, 1);
        return rm;
    }
    
    public static Matrix4x4 Rotate(Quaternion rot) {
        return Quaternion.ToMatrix(rot);
    }
    
    public static Matrix4x4 TRS(Vector3 pos, Vector3 rot, Vector3 scl) {
        var tm = new Matrix4x4();
        tm.c0 = new Vector4(1, 0, 0, pos.x);
        tm.c1 = new Vector4(0, 1, 0, pos.y);
        tm.c2 = new Vector4(0, 0, 1, pos.z);
        tm.c3 = new Vector4(0, 0, 0, 1);

        var rm = Rotate(rot);

        var sm = new Matrix4x4();
        sm.c0 = new Vector4(scl.x, 0, 0, 0);
        sm.c1 = new Vector4(0, scl.y, 0, 0);
        sm.c2 = new Vector4(0, 0, scl.z, 0);
        sm.c3 = new Vector4(0, 0, 0, 1);

        return tm * rm * sm;
    }
    
    public static Matrix4x4 TRS(Vector3 pos, Quaternion rot, Vector3 scl) {
        var tm = new Matrix4x4();
        tm.c0 = new Vector4(1, 0, 0, pos.x);
        tm.c1 = new Vector4(0, 1, 0, pos.y);
        tm.c2 = new Vector4(0, 0, 1, pos.z);
        ...
        var rm = Quaternion.ToMatrix(rot);
    ...
    //      [sy * cx,                   -sx ,          cy * cx         ]
    public static Matrix4x4 InverseTRS(Vector3 pos, Vector3 rot, Vector3 scl) {
        pos = -pos;
    ...
    public static Matrix4x4 InverseTRS(Vector3 pos, Quaternion rot, Vector3 scl) {
        pos = -pos;
        scl = new Vector3(scl.x == 0 ? 0 : 1 / scl.x, scl.y == 0 ? 0 : 1 / scl.y, scl.z == 0 ? 0 : 1 / scl.z);
        ...
        var rm = Transpose(Quaternion.ToMatrix(rot));
    ...
    public static Matrix4x4 Perspective(float fov, float aspect, float zNear, float zFar) {
        ...
        m.c0 = new Vector4(cotHalfFov / aspect, 0, 0, 0);
        m.c1 = new Vector4(0, cotHalfFov, 0, 0);
        m.c2 = new Vector4(0, 0, -(zFar + zNear) / (zFar - zNear), -2 * zFar * zNear / (zFar - zNear));
        m.c3 = new Vector4(0, 0, -1, 0);
        return m;
    }
    ...
    //     [0,                   0,        -2 / (f - n), -(f + n) / (f - n)]
    //     [0,                   0,        0,            1                 ]
    public static Matrix4x4 Ortho(float size, float aspect, float zNear,float zFar) {
        var m = new Matrix4x4();
        m.c0 = new Vector4(1 / (aspect * size), 0, 0, 0);
        m.c1 = new Vector4(0, 1 / size, 0, 0);
        m.c2 = new Vector4(0, 0, -2 / (zFar - zNear), -(zFar + zNear) / (zFar - zNear));
        m.c3 = new Vector4(0, 0, 0, 1);
        return m;
    }
    ...
    public static Matrix4x4 AngleAxis(float angle, Vector3 axis) {
        var nx = axis.x;
        var ny = axis.y;
        var nz = axis.z;
        ...
        var m = new Matrix4x4();
        m.c0 = new Vector4(nx * nx * (1 - ca) + ca, nx * ny * (1 - ca) - nz * sa, nx * nz * (1 - ca) + ny * sa, 0);
        m.c1 = new Vector4(nx * ny * (1 - ca) + nz * sa, ny * ny * (1 - ca) + ca, ny * nz * (1 - ca) - nx * sa, 0);
        m.c2 = new Vector4(nx * nz * (1 - ca) - ny * sa, ny * nz * (1 - ca) + nx * sa, nz * nz * (1 - ca) + ca, 0);
        m.c3 = new Vector4(0, 0, 0, 1);
        return m;
    }
    ...
    public static Vector3 ToEuler(Matrix4x4 m) {
        float x, y, z;
