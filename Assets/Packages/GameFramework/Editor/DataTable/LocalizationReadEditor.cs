...
namespace GameFramework
{
    internal static class LocalizationReadEditor
    {
        ...
        public static void ChangeLanguage(string type)
        {
            ...
            {
                string languageKeyPath = Path.Combine(DataTableSetting.Instance.LoadLocalizationPath, "LanguageKey.bytes");
                if (!FileUtility.Exists(languageKeyPath))
                {
                    Debug.LogError($"Load localization {languageKeyPath} is fail");
                    return;
                }
            ...
            {
                string languagePath = Path.Combine(DataTableSetting.Instance.LoadLocalizationPath, type, "Language.bytes");
                if (!FileUtility.Exists(languagePath))
                {
                    Debug.LogError($"Load localization {languagePath} is fail");
                    return;
                }
