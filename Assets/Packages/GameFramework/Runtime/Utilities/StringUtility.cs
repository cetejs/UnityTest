...
namespace GameFramework
{
    ...
    {
        private static BoyerMoore boyerMoore = new BoyerMoore();

        public static int SearchOfBm(string text, string pattern, bool isIgnoreCase)
        {
            return boyerMoore.Search(text, pattern, isIgnoreCase);
        }
        ...
        private class BoyerMoore
        {
            ...
            private bool GenerateBc(string pattern)
            {
                if (bc == null)
                {
                    bc = new int[256];
            ...
            private void GenerateGs(string pattern)
            {
            ...
            private int MoveByGs(int j, string pattern)
            {
                int m = pattern.Length;
                int k = m - 1 - j;
                if (suffix[k] != -1)
            ...
            public int Search(string text, string pattern, bool isIgnoreCase)
            {
